...
專案採用雙引擎架構，將本機 API 模式（Node 後端）與 AI Studio 直連模式（原 Ultra Lite 專案）完整整合成單一專案。你可以於本地環境啟動本機開發伺服器運行，也可以直接部署／運行於 Google AI Studio 享受 Google AI Pro / Ultra 訂閱額度。工作區支援自動環境偵測，亦可在頂部導覽列隨時手動切換執行引擎（「自動偵測」、「本地 API (Ultra)」、「AI Studio 直連 (Lite)」）。
...
最新版本：4.1.0。工作區佈局比例重構與提示詞操作體驗全面升級。包含桌面端 60/40 比例配置（頂部導覽列 60% / 支援欄 40%、下半部生成輸入區 60% / 歷史紀錄與主舞台 40% 且雙欄等高對齊）、歷史紀錄縮圖 1 頁 4 張、Lite 模式下佇列按鈕動態隱藏（進展與支援按鈕等寬平分）、References 懸浮卡片向下展開、提示詞輸入框手動字級縮放與 LocalStorage 記憶保留、Enter 發送/換行切換鈕圖示化與 34px 等寬垂直對齊，以及提示詞文字與滾動條/按鈕間距整體縮小優化。版本細節請見 [CHANGELOG.md](CHANGELOG.md)。
