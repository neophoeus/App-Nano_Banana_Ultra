...
- Revolve 模式支援 3D 高斯波濺（3DGS）空間視角調整，提供 Orbit/Pan 工具、垂直 Zoom 滑桿（20%~300%）、滾輪縮放、滑鼠右鍵直接平移快捷鍵、獨立 [Src_1] 原圖通道保護使用者參考圖，以及通用多風格提示詞約束
...
4.6.x 引入 Revolve 模式的完整 3D 空間相機互動操作（Orbit/Pan 工具切換、20%~300% 垂直縮放、右鍵直接拖曳平移），並以連續微四邊形 3DGS 渲染消除點陣偽影。將 Revolve 底圖解耦為一級獨立參數通道（`[Src_1]`）並與 3D 視角畫布（`[Edit_1]`）並行傳送，徹底保護使用者自行上傳的參考圖（`[Obj_1]`, `[Obj_2]`），同時採用風格中立、主題通用的精簡 2 句提示詞，相容寫真、動漫、二次元、3D、插畫、水彩等各類創作主題與風格。
