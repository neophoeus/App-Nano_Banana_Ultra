...
- Revolve mode with 3D Gaussian Splatting (3DGS) spatial perspective control, Orbit/Pan tools, vertical Zoom slider (20%~300%), wheel zoom, right-click drag pan shortcut, edge-preserving bilateral depth estimation, 3D surface normal geometry, hardware-accelerated WebGL2 GPU shader rendering with 2D fallback, independent [Src_1] source image channel protecting user references, and universal multi-style prompt conditioning
...
Version 4.7.x standardizes the remote Gemini Batch Queue architecture to strict `@google/genai` v1.31.0+ official API schemas (providing `{ contents, generationConfig }` fallbacks, paginated `/api/batches/list`, remote `/api/batches/delete`, and workspace remote batch recovery), and delivers full parity with the standalone Lite edition by porting 9-language window-close warning confirmations, expanding workspace app lifecycle unit test coverage, and supporting client test service override hooks with Playwright smoke tests. It executes a major architectural modularization of the application shell, decomposing `App.tsx` from 3,044 lines down to 2,482 lines (~562 lines reduced) by cleanly decoupling the top header support rail (`WorkspaceSupportRail.tsx`), stage download and Sidecar JSON metadata construction (`useStageImageDownloadActions.ts`), batch preview session streaming (`useBatchPreviewSession.ts`), and unified detail modal overlays (`WorkspaceDetailOverlays.tsx`) with zero regression in Phase B render stability and 100% automated test pass rate.

### 4.6.x+ (4.x Series)

Version 4.6.x introduces full 3D interactive spatial camera manipulation in Revolve mode (Orbit/Pan tools, 20%~300% zoom, right-click pan shortcut) powered by continuous 3D Gaussian Splatting (3DGS). It delivers Solution A+B: edge-preserving cross-bilateral disparity fields that lock depths onto subject contours, 3D surface normal geometry that tilts splats along surface tangents with occlusion boundary tearing, and a hardware-accelerated WebGL2 GPU shader engine running at a smooth 60 FPS with automatic Canvas 2D fallback. It completely decouples the Revolve source image into an independent first-class channel (`[Src_1]`) alongside the 3D viewpoint guidance (`[Edit_1]`), fully preserving user-uploaded reference images (`[Obj_1]`, `[Obj_2]`) while adopting a universal, style-agnostic 2-sentence prompt contract suitable for any subject and art style with 100% Google AI Studio sandbox compatibility.
