...
- Release summary:
    - **Revolve 3D Gaussian Splatting Spatial Perspective, Pan & Zoom Manipulation (`components/ImageEditor.tsx`, `utils/gaussianSplatting.ts`, `utils/editorPromptBuilder.ts`)**: Upgraded the Revolve editing workflow to full 3D interactive spatial camera manipulation. Added left dock tool toggles between 3D Orbit (Yaw & Pitch rotation) and Pan (spatial hand translation). Added a dedicated vertical Zoom slider (20% ~ 300%) with quick-reset badge and smooth canvas wheel zoom with modifier keys (`Alt` for Yaw, `Shift` for Pitch). Real-time 3D Gaussian point cloud rendering dynamically applies `panX`, `panY`, and `zoom` projection matrices to both live preview and final canvas rendering. Prompt builder automatically derives spatial framing instructions (e.g., camera zoom factors and reframing offsets) for Gemini generation.
    - **Revolve Mouse Right-Click Hand/Pan Drag Shortcut & Native Context Menu Suppression (`components/ImageEditor.tsx`)**: Bound mouse right-click drag (`button === 2` / `buttons === 2`) as an intuitive instant shortcut for 3D hand/pan translation regardless of whether Orbit or Pan tool is selected. Added `onContextMenu` interception with `e.preventDefault()` on the Revolve event surface to completely prevent intrusive browser context menus during right-click panning.
    - **Reframe Toolbar Clean-Up & Dedicated Right Slider Consolidation (`components/ImageEditor.tsx`)**: Streamlined the Reframe top toolbar by eliminating redundant left 90°, right 90°, magnetic snap, and rotation angle badge controls, consolidating rotation adjustments exclusively to the right-hand vertical slider. Clicking the rotation badge on the right panel instantly resets the angle to 0°.
    - **Classic Horseshoe Magnet Snap Icon & 45° Visual Feedback (`components/ImageEditor.tsx`)**: Redesigned the magnetic snap toggle icon into an unmistakable classic U-shaped horseshoe magnet with dual pole dividers and a prominent `45°` badge. Features an amber glowing active state that immediately communicates 45-degree angle snapping.
    - **Rotated Bounding Box Fit & Fill View Preservation (`components/ImageEditor.tsx`)**: Resolved an issue where clicking Fit View (contain) or Fill View (cover) inadvertently reset rotation to 0°. Implemented exact rotated bounding box trigonometric geometry (\(W|\cos\theta| + H|\sin\theta|\) and \(W|\sin\theta| + H|\cos\theta|\)) to preserve the existing rotation angle while recalculating precise contain and cover scale factors.
    - **9-Language Localization Expansion (`utils/translations/*.ts`)**: Added dedicated localized dictionary keys (`revolveToolOrbit`, `revolveToolPan`, `revolveZoom`, `revolvePan`) across all 9 supported languages (`zh_TW`, `zh_CN`, `en`, `ja`, `ko`, `de`, `es`, `fr`, `ru`).
    - **Comprehensive Automated Test Coverage (`tests/ImageEditor.revolve.test.tsx`, `tests/ImageEditor.rotation.test.tsx`, `tests/editorPromptBuilder.test.ts`)**: Added comprehensive unit test suites covering Revolve Orbit/Pan tool switching, mouse drag panning, wheel zooming, right-click pan shortcut, native context menu interception, Reframe rotated Fit/Fill preservation, and magnetic snapping. All 122 test files and 1,013 tests pass at 100% with zero regressions.
